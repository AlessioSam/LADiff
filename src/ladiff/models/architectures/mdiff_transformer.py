...
import math
#from ladiff.models.operator.cross_attention import (TransformerEncoderLayer)
...
class LinearTemporalCrossAttention(nn.Module):
    ...
    def forward(self, x, xf, emb, src_key_padding_mask=None):
        """
        x: B, T, D
        xf: B, N, L
        """
...
class LinearTemporalDiffusionTransformerDecoderLayer(nn.Module):
    ...
    def forward(self, x, xf, emb, src_mask=None, src_key_padding_mask=None):
        ...
        x = self.ca_block(x, xf, emb, src_key_padding_mask=src_key_padding_mask)
        x = self.ffn(x, emb)
        x = x.permute(1, 0, 2)
