...
from ladiff.models.architectures.tools.embeddings import (TimestepEmbedding,
                                                       Timesteps)
from ladiff.models.operator import PositionalEncoding
from ladiff.models.operator.cross_attention import (SkipTransformerEncoder,
                                                 TransformerDecoder,
                                                 TransformerDecoderLayer,
                                                 TransformerEncoder,
                                                 TransformerEncoderLayer)
from ladiff.models.operator.position_encoding import build_position_encoding
from ladiff.utils.temos_utils import lengths_to_mask
from ladiff.models.architectures.mdiff_transformer import LinearTemporalDiffusionTransformerDecoderLayer

class LADiffDenoiser(nn.Module):
    ...
    def forward(self,
        ...
        if not self.test_efficiency:
            if max_iter_elements is not None:
                ...
                latent_mask = torch.ones((len(max_iter_elements), max_iter),
                                        dtype=bool,
                                        device=sample.device)
                for i, max_iter_element in enumerate(max_iter_elements):
                    latent_mask[i, max_iter_element:] = False
        ...
        if self.arch == "trans_enc": #this
            ...
            if self.MD_trans:
                ...
                tokens = self.encoder(src=xseq, xf=text_emb_latent, 
                                      ...
                                      src_key_padding_mask=~latent_mask if (max_iter_elements is not None and not self.test_efficiency)
                                        else None)
