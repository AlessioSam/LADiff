...
def parse_args(phase="train"):
    ...
    if phase == "demo":
        ...
        group.add_argument(
            ...
            action='store_true',
            required=False,
            help="generate one motion per latent, by padding all the subsequent latents with zeros. Put ONLY ONE example if using this mode"
        )
        group.add_argument(
            "--latentwise_gen_bw",
            action='store_true',
            required=False,
            help="generate one motion per latent, by padding all the previous latents with zeros. Put ONLY ONE example if using this mode"
        )
        group.add_argument(
            "--plot_att_map",
            action='store_true',
            required=False,
            help="generate cross attention maps between frames and latents. Put ONLY ONE example if using this mode"
        )
    ...
    if phase == "demo":
        ...
        cfg.DEMO.LATENTWISE_GEN_FW = params.latentwise_gen_fw
        cfg.DEMO.LATENTWISE_GEN_BW = params.latentwise_gen_bw
        cfg.DEMO.PLOT_ATT_MAP = params.plot_att_map
