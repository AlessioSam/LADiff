...
class VQVAE_251(nn.Module):
    ...
    def forward(self, x):
        # TRANSORMER -> QUANTIZ -> SAMPLING NORMAL DIST -> DIFFUSION
        x_in = self.preprocess(x) # (B, 263, T)
        # Encoder
        x_encoder = self.encoder(x_in) # (B, H, L)

        ## quantization
        x_quantized, loss, perplexity  = self.quantizer(x_encoder) # (B, H, L)
