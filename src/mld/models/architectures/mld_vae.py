...
class MldVae(nn.Module):
    ...
    def encode(
            self,
            features: Tensor,
            lengths: Optional[List[int]] = None, remo=None
        ...
        if remo:
            mask = self.generate_src_mask(nframes, lengths).to(device)
        ...
        #if aug_mask.shape[1] < xseq.shape[0]: #!!! ADDED WITH REMODIFFUSION
        #    to_pad = torch.zeros((bs, xseq.shape[1]-aug_mask.shape[1]), dtype=bool, device=x.device)
        #    aug_mask = torch.cat((aug_mask, to_pad), dim=1)
