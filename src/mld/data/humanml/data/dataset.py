...
class Text2MotionDatasetV2(data.Dataset):
    ...
    def __init__(
        ...
        debug=False,
        progress_bar=True,
        phase="train", subphase=False, nframes=8,
        loading_type_t2m=False,
        #force_seq=False,
        **kwargs,
    ):
        self.w_vectorizer = w_vectorizer
        ...
        #self.whats_loading = split_file.split('/')[-1].split('.')[0]
        
        if self.phase == "train":
            if loading_type_t2m:
                self.max_length = 64 #!!20
                self.max_motion_length = 64 #!!max_motion_length
                self.min_motion_length = 64 #!!min_motion_length
            else:
                if subphase=='stage1':
                    self.max_length = self.nframes #64 #!!20
                    self.max_motion_length = max_motion_length #!!max_motion_length
                    self.min_motion_length = min_motion_length #64 #!!min_motion_length
                elif subphase=='stage2':
                    self.max_length = self.nframes*2
            ...
            self.max_length = 20
            self.max_motion_length = max_motion_length #96
            self.min_motion_length = min_motion_length #49

        #if self.whats_loading != 'train':
        #    self.max_length = 20
        #    self.max_motion_length = max_motion_length
        #    self.min_motion_length = min_motion_length
        ...
        self.loading_type_t2m = loading_type_t2m
        ...
        self.name_list = name_list
        #if self.subphase==None or self.whats_loading != 'train':
        self.reset_max_len(self.max_length)
    ...
    def __getitem__(self, item):
        ...
        if self.phase != "train":
            ...
            # if m_length < self.min_motion_length: # TO TEST ONLY FOR SPECIFIC N OF LATENTS
            #     m_length = len(motion)
            ...
            if use_coin:
                ...
                if coin < 0.66:
                    idx = random.randint(0, len(motion) - len(motion))
                else:
                    idx = random.randint(0, len(motion) - m_length)
            else:
                idx = random.randint(0, len(motion) - m_length)
        else:
            if self.loading_type_t2m:
                m_length = self.max_length
                idx = random.randint(0, len(motion) - self.max_length)
            else:
                    if self.subphase!=None or self.subphase!='stage2_1':
                            
                        #if self.whats_loading == 'train':
                            if self.subphase=='stage2':
                                m_length = self.nframes*2
                            elif self.subphase=='stage1':
                                m_length = self.nframes
                            #m_length = self.nframes*2 if self.subphase=='stage2' else self.nframes
                #    idx = random.randint(0, len(motion) - self.max_length)
                    ...
                    # Sample a number from 0 and 1 with uniform distribution
                    use_coin = True
                    if use_coin:
                        coin = np.random.uniform(0, 1)
                        if coin < 0.66:
        ...
        motion = motion[idx:idx + m_length] #!! added?? /1000
        ...
        #if self.force_seq:
        #    # Chunk the motion into sequences of length cfg.model.seq_len
        #    motion_chunks = torch.split(motion, self.cfg.model.seq_len, dim=0)
        #    print(motion_chunks.shape)
    ...
    def __init__(self,
        ...
        if mode == "text_only":
            ...
            self.t2m_dataset = Text2MotionDatasetV2(self.opt, self.mean,
                                                    ...
                                                    self.w_vectorizer, evaluation=True)
