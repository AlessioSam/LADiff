...
def get_mean_std(phase, cfg, dataset_name):
    ...
    if phase in ["val"]:
        ...
        #    mean = np.load('./deps/t2m/kit/VQVAEV3_CB1024_CMT_H1024_NRES3/meta/mean.npy')
        #    std = np.load('./deps/t2m/kit/VQVAEV3_CB1024_CMT_H1024_NRES3/meta/std.npy')
            #mean = np.load(pjoin(data_root, "Mean.npy"))
            #std = np.load(pjoin(data_root, "Std.npy"))
...
def get_datasets(cfg, logger=None, phase="train"):
    ...
    for dataset_name in dataset_names:
        if dataset_name.lower() in ["humanml3d", "kit"]:
            #if cfg.DATASET.LOAD_LATENTS!=None:
            #    dataset_name_to_load = cfg.DATASET.LOAD_LATENTS
            ...
            dataset = dataset_module_map[dataset_name.lower()](
                ...
                phase=phase, subphase=cfg.TRAIN.SUBPHASE,
                nframes=cfg.TRAIN.N_FRAMES,
                loading_type_t2m=cfg.model.vae_from_t2m,
                #force_seq=cfg.FORCE_SEQ
            )
