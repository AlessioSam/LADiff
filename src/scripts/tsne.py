...
def main():
    ...
    with torch.no_grad():
        texts = ['a man walks in circle']
        cond_emb = text_encoder(texts).to(device) # [1, emb_dim]

        
        #labels = torch.tensor(np.array(list(range(0,12)))).unsqueeze(1).to(device) # [classes, 1]
        lengths = torch.tensor([60]*len(texts)).to(device) # [classes]
        ...
        z_list = []
        for i in track(range(10),'Generating latent codes'):
    ...
    # Draw figure
    drawFig(output_dir, latents, classids = [8,6,5], steps = [0, 3,6,9])
