...
def main():
    ...
    latentwise_gen_type = None
    if cfg.DEMO.LATENTWISE_GEN_FW and cfg.DEMO.LATENTWISE_GEN_BW:
        print("Error: cannot use both --latentwise_gen_fw and --latentwise_gen_bw")
        quit()
    elif cfg.DEMO.LATENTWISE_GEN_FW:
        latentwise_gen_type = "fw"
    elif cfg.DEMO.LATENTWISE_GEN_BW:
        latentwise_gen_type = "bw"
    ...
    if cfg.ACCELERATOR == "gpu":
        ...
        device = torch.device("cuda:"+str(cfg.TEST_DEVICE[0]))
        ...
        if text:
            ...
            for rep in range(cfg.DEMO.REPLICATION):
                ...
                # text to motion synthesis
                    ...
                    joints = model(batch, latentwise_gen=latentwise_gen_type, plot_att_map=output_dir if cfg.DEMO.PLOT_ATT_MAP else None)
                    
                    if latentwise_gen_type:
                        length = length*cfg.TRAIN.ABLATION.MAX_IT
                        text = text*cfg.TRAIN.ABLATION.MAX_IT
                        batch = {"length": length, "text": text}
                ...
                for i in range(nsample):
                    npypath = str(output_dir /
                                f"{task}_{length[i]}_batch{id}_{'latents'+str(i+1) if latentwise_gen_type else i}.npy")
                ...
                if cfg.DEMO.OUTALL: # it means "output all samples in one file", don't use it when using --latentwise_generation
